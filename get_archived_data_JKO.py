...
if os.environ.get("METRANETLIB_PATH") is None:
    os.environ["METRANETLIB_PATH"] = f"/scratch/mch/mryan/metranet_lib/"
    #os.environ["METRANETLIB_PATH"] = f"/scratch/mch/maregger/hailclass/hailclass_data/metranet_lib"


BASE_DIR_RADAR = (
    "/store_new/mch/msrad/radar/swiss/data/"  # on MCH servers: "/repos/repos_prod/radar/swiss"
)
#TEMP_ZIP_DIR = f"/scratch/{username}/zip_temp_dir"
TEMP_ZIP_DIR = f"/users/mryan/Code/zip_temp_dir" 
#TEMP_GRID_DIR = f"/scratch/{username}/grid_temp_dir"
TEMP_GRID_DIR = f"/users/mryan/Code/grid_temp_dir"

# TODO: Properly solve the situation with the necessary files within the repository
MCH_NC_EXAMPLE = f"/users/{username}/code/radar_tools/BZC_6t6_2022.nc"
...
def build_zip_file_paths(
    ...
    if product in ["EZC15", "EZC45", "EZC50"]:
...
def prepare_gridded_radar_data_from_zip(
    ...
    if product in ["CPCH_5", "CPCH_60"]:
        ...
        if reader == "C":
            values = rad.read_file(file=file_path_out, physic_value=True).data    ### rad.read_file function is unknown: The function is not in the radlib package!!
        if reader == "Python":
...
def save_multiple_radar_grids(
    ...
    while temp_date < end_date:
        ...
        np.save(
            "/scratch/%s/temp/subdaily_npy/%s_%s.npy"
